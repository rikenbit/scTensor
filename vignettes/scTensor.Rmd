...
Here, we extend the similar approarch to the construction of the L-R-pair lists
...
L-R database of Homo sapiens). Besides, we also developed
(`r Biocpkg("scTensor")`), whichis a method to 
detect CCI and the CCI-releated L-R pairs simutaneously.
...
respectively. Finally, the L-R-pairs which is registed in STRING are extracted
...
"meta"-packaging. In this case, only user have to specify are 1. a L-R-list
...
3 ligand-patterns, 3 receptor-patterns, and 15 L-R-pairs-patterns.
...
cellCellReport(sce, reducedDimNames="TSNE",
    ...
    author="Koki Tsuyuzaki", thr=15, top=2, html.open=FALSE)
```
